...
pub fn get_intel_cert(server_param: &ServerParam) -> Option<rustls::ServerConfig> {
    // Generate Keypair
    let ecc_handle = SgxEccHandle::new();
    let _result = ecc_handle.open();
    let (prv_k, pub_k) = ecc_handle.create_key_pair().unwrap();

    let (attn_report, sig, cert) = match create_attestation_report(&pub_k, server_param) {
        Ok(r) => r,
        Err(e) => {
    ...
    let mut root_ca_bin = Vec::new();
    {
        let intel_ca = fs::File::open(server_param.get_intel_ca()).expect("intel_ca file not found");
        BufReader::new(intel_ca).read_to_end(&mut root_ca_bin).expect("read intel_ca file failed.");
    }
    let mut ca_reader = BufReader::new(root_ca_bin.as_slice());

    //let root_ca_bin = include_bytes!(intel_ca);
    ...
    let mut tls_config = rustls::ServerConfig::new(authenticator);
...
fn create_attestation_report(
    ...
    server_param: &ServerParam) -> Result<(String, String, String), sgx_status_t> {
    // Workflow:
    ...
    let sigrl_vec: Vec<u8> = get_sigrl_from_intel(ias_sock, eg_num, server_param);
    ...
    let (attn_report, sig, cert) = get_report_from_intel(ias_sock, quote_vec, server_param);
    Ok((attn_report, sig, cert))
}

fn get_sigrl_from_intel(fd: c_int, gid: u32, server_param: &ServerParam) -> Vec<u8> {
    info!("get_sigrl_from_intel fd = {:?}", fd);
    let config = make_ias_client_config(server_param);
...
fn get_report_from_intel(fd: c_int, quote: Vec<u8>, server_param: &ServerParam) -> (String, String, String) {
